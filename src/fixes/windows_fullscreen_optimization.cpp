...
{
    if (Util::IsSteamOS() || !(eGameType & (MGS4 | MGSPW)))
    {
        return;
    }

    const auto markerFile = sGameSavePath / "MGSPatriotFix_fullscreen_optimization.bin";
        ...
        if(!markerExists)
        {
            try
            {
                ...
                if (out)
                {
                    ...
                    out << "MGSPatriotFix wrote this file to track fullscreen optimization registry state.\n";
